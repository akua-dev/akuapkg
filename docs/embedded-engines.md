...
1. **Single binary UX.** A published Akuapkg binary includes the supported engines. You do not need separate Helm, OPA, or KCL installations.
...
Agents get the full engine toolkit from one Akuapkg installation with zero engine-specific PATH management. Skills that invoke `akuapkg test`, `akuapkg fmt`, or `akuapkg bench` do not need to check `which opa`.
