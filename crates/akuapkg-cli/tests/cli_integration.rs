...
//!
//! These drive the compiled `akuapkg` binary (not the library surface),
...
/// Run `akuapkg <args>` in `cwd` and return (exit-code, stdout, stderr).
///
...
fn run(cwd: &Path, args: &[&str]) -> Output {
    Command::new(AKUAPKG_BIN)
        .current_dir(cwd)
        .env("AKUA_NO_AGENT_DETECT", "1")
        .args(args)
        .output()
        .expect("spawn akuapkg binary")
}
...
fn init_then_render_without_inputs_flag_uses_scaffold_inputs_example() {
    // After `akuapkg init`, the scaffold drops `inputs.example.yaml` next
    // to package.k. `akuapkg render` without --inputs should auto-discover
...
fn path_dep_to_akua_package_resolves_through_tree() {
    ...
    // dep system. `akuapkg tree --json` reports resolved deps; without
...
fn lock_rejects_helm_dep_referenced_via_import() {
    ...
    // cleanly and the failure defers to `akuapkg check`'s opaque
    // CannotFindModule.
    //
...
fn agent_env_flips_output_mode_to_json_automatically() {
    ...
    // defeat this test.
    let dir = tempdir();
    let out = Command::new(AKUAPKG_BIN)
