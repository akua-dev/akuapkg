...
//! topology cycle-free — akuapkg-cli depends on the precompiled .cwasm,
...
fn main() {
    ...
    if !worker_wasm.exists() {
        ...
        // first `akuapkg render` post-install. Better to fail the build.
        //
        // dev / test profiles still get the empty-sandbox fallback
        // so contributors who haven't run `task build:render-worker`
        // yet aren't blocked from compiling akuapkg-cli for unit tests
        ...
        if release_like {
            panic!(
                "akua-render-worker.wasm not found at {} — release profiles must ship a worker. \
                 ...
                 `cargo build -p akuapkg-cli --release`.",
                worker_wasm.display()
            );
        }
        println!(
    ...
    // from the same akua-core sources akuapkg-cli is currently compiling
...
/// akuapkg-cli's build.rs runs with CWD = crates/akuapkg-cli. The workspace
...
fn workspace_root() -> PathBuf {
    ...
    PathBuf::from(manifest_dir)
        .parent()
        .and_then(|p| p.parent())
        .expect("crates/akuapkg-cli parent chain")
        .to_path_buf()
}
