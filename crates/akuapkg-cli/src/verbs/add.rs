//! `akuapkg add` — insert a dependency into `akua.toml`.
//!
        ...
        match self {
            AddError::Load(e) => {
                ...
                if matches!(e, ManifestLoadError::Missing { .. }) {
                    base.with_suggestion("run `akuapkg init` first")
                } else {
                    base
                }
            }
...
pub fn run<W: Write>(
    ...
    // `akuapkg add` is the verb where OCI pulls + lockfile updates are
    ...
    let opts = ResolverOptions {
        ...
        cache_root: None,
        expected_digests,
        cosign_public_key_pem: None, // akuapkg add surfaces cosign via render / verify
    ...
    match chart_resolver::resolve_with_options(&manifest, args.workspace, &opts) {
        ...
        Err(_soft) => {
            ...
            // declarative intent. `akuapkg render` re-runs the resolver
