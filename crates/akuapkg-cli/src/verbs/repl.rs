//! `akuapkg repl` — interactive KCL shell.
//!
...
//!   history + arrow-keys can wrap with `rlwrap akuapkg repl`.
...
//! mode. Agents invoking `akuapkg repl` programmatically should use
//! `akuapkg render` or `akuapkg inspect` instead.
//!
//! ## KCL quirks
//!
...
pub fn run<R: BufRead, W: Write>(
    ...
    if matches!(ctx.output, crate::contract::OutputMode::Json) {
        writeln!(
            stdout,
            "{{\"note\":\"akuapkg repl is interactive; JSON output is not supported — falling back to text\"}}"
        )
        .map_err(ReplError::StdoutWrite)?;
    } else {
        writeln!(
            stdout,
            "akuapkg repl — KCL interactive (type `.exit` or Ctrl-D to quit, `.help` for commands)"
        )
        .map_err(ReplError::StdoutWrite)?;
    }
    ...
    loop {
        ...
        if n == 0 {
            ...
            // scripts (`echo "x = 42" | akuapkg repl` → one eval + exit).
