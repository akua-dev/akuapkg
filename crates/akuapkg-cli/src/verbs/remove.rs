//! `akuapkg remove` — drop a dependency from `akua.toml` + `akua.lock`.
//!
...
//! re-fetch — a separate `akuapkg cache gc` verb can reap them later).
...
pub fn run<W: Write>(
    ...
    if removed {
        ...
        // Prune matching lockfile entries too so `akuapkg verify` stays
