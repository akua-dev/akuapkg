//! `akuapkg pull` — retrieve a published akua Package from an OCI registry
//! and extract it to a target directory.
//!
//! Inverse of `akuapkg publish`. The resolved manifest digest is emitted
...
pub struct PullArgs<'a> {
    ...
    /// Tag to pull. Required — unlike `akuapkg publish`, there's no
