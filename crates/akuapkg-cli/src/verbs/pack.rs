//! `akuapkg pack` — build the same tarball `akuapkg publish` uploads, but
...
//! as `akuapkg publish`), so re-packing an unchanged workspace produces
...
pub struct PackArgs<'a> {
    ...
    /// subdir is walker-skipped, so repeated `akuapkg pack` runs
