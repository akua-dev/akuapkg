//! `akuapkg cache` — list, clear, or locate the content-addressed caches
//! that `akuapkg add` + `akuapkg render` populate on disk.
//!
//! Subverbs:
//! - `akuapkg cache list` — enumerate OCI blobs + git repos/checkouts +
//!   helm charts under `$XDG_CACHE_HOME/akua/{oci,git,helm}` with sizes.
//! - `akuapkg cache clear [--oci | --git | --helm]` — reclaim disk. Default
//!   wipes all three; flags narrow it. Safe on absent caches (no-op).
//! - `akuapkg cache path` — print the resolved cache roots. Useful for
//!   scripting `du -sh` / mount-point pinning on CI runners.
//!
//! Why this exists: ephemeral CI runners + self-hosted agents share
//! disk across tenants. "How big is the akuapkg cache?" and "nuke the
