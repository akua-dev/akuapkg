//! `akuapkg verify` — lockfile consistency check.
//!
//! Spec: [`docs/cli.md`](../../../../docs/cli.md) `akuapkg verify` section.
//!
        ...
        match self {
            VerifyError::Manifest(e) => {
                ...
                if matches!(e, ManifestLoadError::Missing { .. }) {
                    base.with_suggestion("run `akuapkg init` or check the working directory")
                } else {
                    base
                }
            }
            VerifyError::Lock(e) => {
                ...
                if matches!(e, LockLoadError::Missing { .. }) {
                    base.with_suggestion(
                        "run `akuapkg add` to resolve deps and generate the lockfile",
                    )
                } else {
                    base
                }
...
pub fn check(workspace: &Path) -> Result<VerifyOutput, VerifyError> {
    ...
    // vendored tree without re-running `akuapkg add` — CI must fail. We
    // run the resolver in offline mode so OCI/git deps don't touch
    // the network from `akuapkg verify`.
    ...
    // surface as distinct violations so `akuapkg verify --json` gives
        ...
        for v in &output.violations {
            match v {
                ...
                Violation::PathDigestDrift {
                    ...
                    stdout,
                    "  - path-digest-drift: {name}\n      expected {expected}\n      actual   {actual}\n      run `akuapkg add --force {name} --path <current_path>` to refresh"
                )?,
