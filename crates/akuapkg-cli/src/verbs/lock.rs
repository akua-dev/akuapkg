//! `akuapkg lock` — regenerate `akua.lock` from `akua.toml`.
//!
...
pub enum LockError {
    ...
    #[error(
        "lockfile drift detected — on-disk akua.lock doesn't match what `akuapkg lock` would write"
    )]
    Drift,
    ...
    /// the user gets a clear message before `akuapkg check` fails
