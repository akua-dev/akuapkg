//! `akuapkg dev` — file-watch + hot re-render loop.
//!
...
pub struct DevArgs<'a> {
    ...
    /// matching `akuapkg render` auto-discovery.
...
pub fn run<W: Write>(
    ...
    // production), matching `akuapkg render`. `dev` doesn't resolve root
    ...
    R: FnMut(&[PathBuf]) -> Result<String, String>,
{
    ...
    akua_core::dev::watch_and_render(
        ...
        |event| {
            // Broken-pipe handling: if the user pipes `akuapkg dev |
...
fn render_once(
    ...
    // `akuapkg dev` today doesn't resolve `[dependencies]` or expose
...
mod tests {
    ...
    /// `akuapkg render` and `akuapkg dev` share this fallback behavior.
    #[test]
