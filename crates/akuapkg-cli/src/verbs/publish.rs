//! `akuapkg publish` — tarball the workspace + push it to an OCI registry.
//!
//! The reciprocal of `akuapkg add`: where add consumes a registry-hosted
//! chart, publish *produces* one. Shape:
//!
//! ```text
//! akuapkg publish --ref oci://ghcr.io/acme/my-pkg [--tag 0.2.0]
//! ```
//!
...
mod tests {
    ...
    /// Stand up a mock registry that accepts a full `akuapkg publish`:
