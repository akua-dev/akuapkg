//! `akuapkg render` — execute a Package against inputs and write raw YAML manifests.
//!
...
pub struct RenderArgs<'a> {
    ...
    /// Designed for air-gapped CI runners where the `akuapkg add` step
    /// happened elsewhere.
    pub offline: bool,
...
/// so `akuapkg render` + `akuapkg dev` share the probe order.
...
fn resolve_package_charts(
    ...
    let expected_digests = match AkuaLock::load(workspace) {
        ...
        Err(_) => Default::default(), // lock corruption surfaces via `akuapkg verify`
    };
