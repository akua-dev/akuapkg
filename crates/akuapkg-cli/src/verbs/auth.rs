//! `akuapkg auth` — manage credentials in `$XDG_CONFIG_HOME/akua/auth.toml`.
//!
//! Subverbs:
//! - `akuapkg auth list` — enumerate every configured registry across
//!   both akua/auth.toml and ~/.docker/config.json. Never prints the
//!   secret — only `{registry, source, auth_kind}`.
//! - `akuapkg auth add --registry <host> --username <u>` — reads the
//!   password from stdin (operator-safe; mirrors
//!   `docker login --password-stdin`). Writes to akua/auth.toml.
//! - `akuapkg auth add --registry <host> --token` — reads a bearer
//!   token from stdin. Used for GHCR classic PATs, DockerHub PATs.
//! - `akuapkg auth remove --registry <host>` — drop the entry.
//!
...
fn trim_secret(s: &str) -> String {
    ...
    // `echo | akuapkg auth add ...` commonly. Don't strip whitespace
...
pub enum AuthVerbError {
    #[error("no akuapkg auth config path — set $XDG_CONFIG_HOME or $HOME")]
    NoConfigPath,
    ...
    #[error("empty secret — `akuapkg auth add` requires a non-empty value on stdin")]
    EmptySecret,
