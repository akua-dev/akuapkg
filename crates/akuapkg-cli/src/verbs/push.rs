//! `akuapkg push` — upload a pre-packed `.tar.gz` to an OCI registry.
//!
//! The push half of `akuapkg publish`. Pair with `akuapkg pack` to get the
...
//! read. `akuapkg publish` is the all-in-one verb for the common case;
//! `akuapkg push` is for operators who already have a tarball in hand.
//!
//! Unlike `akuapkg publish`, `--tag` is required — the tarball has no
...
fn read_and_validate_sidecar(
    ...
    // different tarball (or an akuapkg version whose config blob has
    // moved).
