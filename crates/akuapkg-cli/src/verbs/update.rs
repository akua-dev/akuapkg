//! `akuapkg update` — intentionally bump `akua.lock` against whatever
//! upstream now serves.
//!
//! Distinct from [`super::lock`]: where `akuapkg lock` fails hard on
//! OCI digest drift (security — registry served different bytes than
//! the last pinned digest), `akuapkg update` accepts the drift and
