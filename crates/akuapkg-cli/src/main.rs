...
use akuapkg_cli::contract::{emit_error, Context, UniversalArgs};
#[cfg(feature = "dev-watch")]
use akuapkg_cli::verbs::dev as dev_verb;
...
use akuapkg_cli::verbs::{sign as sign_verb, verify_tarball as verify_tarball_verb};
...
fn main() {
    ...
    let _observability = akuapkg_cli::observability::init_subscriber(args, &ctx);
...
fn resolve_auth(
    auth_pairs: &[String],
    auth_file: Option<&std::path::Path>,
) -> Result<Option<akua_core::host_auth::HostAuthMap>, akuapkg_cli::auth_parse::AuthParseError> {
    let from_file = auth_file
        .map(akuapkg_cli::auth_parse::load_auth_file)
        .transpose()?;
    let from_flags = akuapkg_cli::auth_parse::parse_auth_pairs(auth_pairs.iter())?;
    Ok(akuapkg_cli::auth_parse::merge_auth(from_file, from_flags))
}
