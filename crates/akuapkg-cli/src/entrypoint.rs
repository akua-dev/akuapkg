...
use crate::verbs::dev as dev_verb;
use crate::verbs::{
...
use crate::verbs::{sign as sign_verb, verify_tarball as verify_tarball_verb};
...
/// Run the complete package command surface from an explicit argument vector.
///
/// This is the single parser/dispatcher used by the standalone `akuapkg`
/// executable and native embedders. It deliberately retains the existing
/// process stdout/stderr contract so embedding cannot silently change any
/// package verb's output, errors, or exit code.
...
///
/// The embedding process owns logging setup; package output and exit semantics
/// stay byte-for-byte identical to the standalone command.
pub fn run_embedded_from<I, T>(args: I) -> ExitCode
where
    I: IntoIterator<Item = T>,
    T: Into<OsString> + Clone,
{
...
fn resolve_auth(
    auth_pairs: &[String],
    auth_file: Option<&std::path::Path>,
) -> Result<Option<akua_core::host_auth::HostAuthMap>, crate::auth_parse::AuthParseError> {
    let from_file = auth_file
        .map(crate::auth_parse::load_auth_file)
        .transpose()?;
    let from_flags = crate::auth_parse::parse_auth_pairs(auth_pairs.iter())?;
    Ok(crate::auth_parse::merge_auth(from_file, from_flags))
}
            ...
            if audience != "public" {
                return emit_structured(
                    ...
                    &StructuredError::new(
                        ...
                        format!(
                            "`akuapkg api spec --audience {audience}` is not supported until the hosted API serves authorized audience-specific OpenAPI documents"
                        ),
                    )
                    .with_default_docs(),
                    ExitCode::UserError,
...
/// Decide where `akuapkg init` writes and what `[package].name` it records.
///
/// Four cases:
/// 1. No name → CWD + sanitized basename (`mkdir foo && cd foo && akuapkg init`).
...
mod tests {
    ...
    fn embedded_runner_preserves_clap_user_error_exit_code() {
        assert_eq!(
            run_embedded_from(["akuapkg", "not-a-command"]),
            ExitCode::UserError
        );
    }
    ...
    fn derive_init_with_path_arg_uses_path_target_and_sanitized_basename() {
        // `akuapkg init ./Some.Subdir` → target `./Some.Subdir`, name
