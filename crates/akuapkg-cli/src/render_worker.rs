...
pub enum WorkerError {
    ...
    #[error(
        "render sandbox unavailable — worker module wasn't compiled into this akua binary. \
         Run `task build:render-worker` and rebuild akuapkg-cli."
    )]
    SandboxUnavailable,
...
impl RenderHost {
    ...
    /// `akuapkg dev`'s watch loop that adds up. Init once on first
...
/// host-registered plugin handlers in akuapkg-cli; the render worker
...
mod tests {
    ...
    /// `task build:render-worker && cargo test -p akuapkg-cli` to get
    /// real coverage.
